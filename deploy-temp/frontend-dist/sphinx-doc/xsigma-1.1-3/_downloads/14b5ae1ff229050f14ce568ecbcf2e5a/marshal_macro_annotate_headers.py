...
This script opts in xsigma classes for automatic marshalling.
...
MARSHAL_HINT_REGEX = r"XSIGMA_MARSHAL(AUTO|MANUAL)"
MARSHAL_HINT_REGEXES = [r"XSIGMA_MARSHALAUTO", r"XSIGMA_MARSHALMANUAL"]
MODULE_HEADER_REGEX = r'^#include "xsigma.*Module\.h"'
MODULE_EXPORT_REGEX = r"^class XSIGMA.*_EXPORT"
WRAPHINT_HEADER_REGEX = r'^#include "xsigmaWrappingHints\.h"'

SCRIPT_DIR = pathlib.Path(os.path.dirname(__file__))
XSIGMA_DIR = SCRIPT_DIR.parent.parent

MARSHALAUTO_TXT = SCRIPT_DIR.joinpath("XSIGMA_MARSHALAUTO.txt")
MARSHALMANUAL_TXT = SCRIPT_DIR.joinpath("XSIGMA_MARSHALMANUAL.txt")
...
def is_valid_line(line, line_num, path):
    if len(line.split("/")) < 3:
        print(f"ERROR: No. of components must be greater than 3 in {path}:{line_num}")
        print(f"ERROR: Got {line}. Expected 'Module/Name/xsigmaClassName.h'")
        return False
...
XSIGMA_MARSHALAUTO.txt (or) XSIGMA_MARSHALMANUAL.txt (or) ignore.txt
...
def get_status():
    ...
    # fail if any of the headers in `XSIGMA_MARSHAL(AUTO|MANUAL).txt` don't have marshal macro
    for marshal_file, macro_regex in zip(
        [MARSHALAUTO_TXT, MARSHALMANUAL_TXT], MARSHAL_HINT_REGEXES
    ):
        with open(marshal_file, "r", encoding="utf8") as f:
            ...
            for line_num, line in enumerate(lines, start=1):
                if not is_valid_line(line, line_num, marshal_file):
                    ...
                    parts = line.split("/")
                    module_dir = XSIGMA_DIR.joinpath(*parts[0:-1])
                    header = module_dir.joinpath(parts[-1])
                    headers[marshal_file].add(header)
                    existing_macro_line_match = find_matching_line(header, macro_regex)
                    if existing_macro_line_match is None:
                        print(f"ERROR: {header} is missing {marshal_file.stem} macro")
                        success &= False

    # fail if the header is in `ignore.txt` and uses a marshal macro (excluding `xsigmaWrappingHints.h`)
    # fail if the header is not in any of `XSIGMA_MARSHAL(AUTO|MANUAL).txt` and `ignore.txt`
        ...
        for filename in files:
            if filename.endswith(".h"):
                ...
                if header in headers[IGNORE_TXT]:
                    ...
                    if (
                        existing_macro_line_match is not None
                        and module_export_line_match is not None
                    ):
                        print(
                    ...
                    if header not in headers[MARSHALMANUAL_TXT]:
                        print(f"ERROR: {header} is missing from {IGNORE_TXT}")
                        success &= False
...
"""
Adds XSIGMA_MARSHAL(AUTO|MANUAL) macro to header files listed in `XSIGMA_MARSHAL(AUTO|MANUAL).txt`
Also adds a line that includes `xsigmaWrappingHints.h` if not already included.
"""
...
def update():
                ...
                if has_exported_class and existing_macro_line_match is not None:
                    ...
                    if has_exported_class and module_header_line_match is not None:
                        ...
                        with open(header, "r", encoding="utf8") as f:
                            ...
                            # Replaces "class XSIGMA*_EXPORT ..." with "class XSIGMA*_EXPORT XSIGMA_MARSHAL(AUTO|MANUAL) ..."
                            export_line_num, export_text_match = (
                                module_export_line_match
                            )
                            export_line_text = new_lines[export_line_num - 1]
                            # Remove existing macro
                            export_line_text = export_line_text.replace(
                                MARSHAL_HINT_REGEXES[0] + " ", ""
                            ).replace(MARSHAL_HINT_REGEXES[1] + " ", "")
                            target = export_text_match.group()
                            replacement = f"{target} {marshal_file.stem}"
                            new_lines[export_line_num - 1] = export_line_text.replace(
                                target, replacement
                            )
                            # Includes xsigmaWrappingHints.h below the include of the xsigma*Module.h
                            ...
                            if wrap_hint_header_line_match is not None:
                                ...
                                new_lines[dst_line] = (
                                    f'#include "xsigmaWrappingHints.h" // For {marshal_file.stem}\n'
                                )
                            else:
                                new_lines.insert(
                                    module_header_line_match[0],
                                    f'#include "xsigmaWrappingHints.h" // For {marshal_file.stem}\n',
                                )
                        # Writes the final result to the file
                        if len(new_lines):
                            with open(header, "w", encoding="utf8") as f:
                                print(f"Write {marshal_file.stem} in {header}")
    ...
    for ignored_header in headers[IGNORE_TXT]:
        ...
        with open(ignored_header, "r", encoding="utf8") as f:
            ...
            has_exported_class = module_export_line_match is not None
            # Replaces "class XSIGMA*_EXPORT XSIGMA_MARSHAL(AUTO|MANUAL) ..." with "class XSIGMA*_EXPORT ..."
            if has_exported_class and existing_macro_line_match is not None:
                new_lines = f.readlines()
                export_line_num, export_text_match = module_export_line_match
                export_line_text = new_lines[export_line_num - 1]
                target = existing_macro_line_match[1].group()
                new_lines[export_line_num - 1] = export_line_text.replace(target, "")
            # Removes xsigmaWrappingHints.h below the include of the xsigma*Module.h
        ...
        if len(new_lines):
            with open(ignored_header, "w", encoding="utf8") as f:
                print(f"Remove marshal macro in {ignored_header}")
                f.writelines(new_lines)

    # Adds all headers not found in `XSIGMA_MARSHAL(AUTO|MANUAL).txt`
    # into `ignore.txt`
    ignore_files = []
    for module_dir in module_dirs:
        ...
        for filename in filenames:
            ...
            if not filename.endswith(".h"):
                continue
            if (
                filepath not in headers[MARSHALAUTO_TXT]
                and filepath not in headers[MARSHALMANUAL_TXT]
            ):
                line = "/".join(filepath.relative_to(XSIGMA_DIR).parts)
